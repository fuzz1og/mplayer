 ...
 *  core 只负责排序、错位检查与 applySwap；log 为可选诊断钩子 */
...
/**
 * 302 端点 id 参数与歌曲 ID 一致性校验（**零请求**）：
 * 源数据可能错位（url 指向另一首歌，如 DJ 版链接实际是纯音乐）——
 * 302 的 id 与候选 song.id 明显不符时标记为可疑（失效）。
 */
...
export function applySwap(_song: Song, source: SourceKey, candidate: SwapCandidate): Song | null {
  ...
  // url 允许为空：路由时代候选天生无 url，播放时经 resolvePlayableSongRouted 现解析。
