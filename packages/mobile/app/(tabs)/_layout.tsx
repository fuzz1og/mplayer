...
import ChromeSurface from '../../components/ChromeSurface';
...
/**
 * 底部 chrome（tab 栏 + 迷你播放栏），在 TabLayout 顶层、内容区之外渲染。
 * 这样 tab 栏的 ChromeSurface / PlayerBar 的 ChromeSurface 都不是自身 blurTarget 的后代，
...
export default function TabLayout() {
  ...
  return (
    <Animated.View style={[styles.container, { backgroundColor: animatedBg }]}>
      ...
      {/* 内容区（悬浮 chrome 不再需要 blurTarget：毛玻璃已改为主题纯色+渐变） */}
      <View style={{ flex: 1 }}>
        <Tabs
      ...
      {/* 顶部 chrome 悬浮（M2）：内容从半透明 TopBar 下穿过 */}
      <View pointerEvents="box-none" style={styles.topChrome}>
        <TopBar />
      </View>
      {/* 底部 chrome（tab 栏 + 迷你播放栏） */}
      <BottomChrome />
    </Animated.View>
  );
}
...
const makeTabBarStyles = () => StyleSheet.create({
  // 毛玻璃由 ChromeSurface 提供（ADR-0005/0010），容器仅排版；overflow hidden 防模糊越界
