...
describe('来源开关 settings IPC（T01 持久化接线）', () => {
  ...
  it('settings:getSourceModes 返回模式 + 每源直连状态（已注册 netease/qianqian/migu/qq → ready，未注册 → unavailable）', async () => {
    registerSettingsIpc();
    ...
    // T02/T04/T05/T06：网易/千千/咪咕/QQ 直连客户端已注册（src/main/api/musicApi.ts 模块加载时），
    // 设置页「直连可用」状态自动变亮；未注册的 kugou 仍为 unavailable。
