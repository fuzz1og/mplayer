...
import { calculateSimilarity, isExactMatch, stripSourceIdPrefix } from '@mplayer/core';
...
export async function searchSwapCandidates(
  ...
  try {
    ...
    console.warn(`换源搜索失败: 《${song.name}》 → ${source}`, error);
    return [];
  }
}
 ...
 * 源数据可能错位（url 指向另一首歌）——api.php?get=url&id=XXX 的 id
 * 与候选 song.id 不符时标记为可疑（失效）。
 */
...
/**
 * 批量探测候选可播性（主进程 HEAD 跟随 302 → CDN 直链，不下载 body）：
