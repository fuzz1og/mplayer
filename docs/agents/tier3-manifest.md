...
设置页的「跳过」计数 = 因归属不匹配被跳过的次数，用来区分「源没命中」和「源被归属过滤」。**跳过与护栏拒绝都不进健康度**（跳过 = 我们没问它；护栏拒绝 = 上游内容与这首歌不匹配），只计数 + 写 trace。

### 上游可按 keyword 搜索的 `url-resolver`：跨源兜底怎么表达

若某条 `url-resolver` 的上游**本身支持按 keyword 搜索**（酷我类端点常见），它其实能服务**任意来源**的歌 —— 但**不能**靠「去掉 `source` 声明」达成：未声明的 `url-resolver` 会被直接拒绝，因为它的 `{id}` 来自原歌的私有 ID 空间。

正确写法是**改写成 `search-then-resolve` 并去掉 `source`**：补 `search` 段（`{keyword}` → 候选，`idPath` 取候选自己的 id），`resolve` 里的 `{id}` 即**该候选自己的 id**（不跨 ID 空间，故合法）。保守做法是保留原有的按源条目、**另加一条同上游的未声明 `search-then-resolve`**（思路同下节「聚合端点」的拆分）。

代价：未声明的 `search-then-resolve` 对**每一首**进 tier3 的歌都会被尝试（多一次搜索请求）；错播风险由歌名 + 歌手精确匹配护栏兜住。
...
上游常见「HTTP 200 + `{code, message}` 业务封套」。tier3 的判定顺序是**先取值、后判封套**（#400）：

1. 先按 `responseJsonPath` 取候选 URL，并过 `allowedDomains`；
2. **只有在取不到可用 URL 时**，才检查顶层 `code`（非 0 数字）+ `message`（非空字符串），命中则记 warn「上游返回错误」，用于区分「上游挂了」与「无此歌」。

因此 `{"code":200,"message":"成功","url":"…"}` 这类**用 HTTP 风格 code 表达成功**、同时带 `message` 字段的上游不会被误判——响应里有没有合法直链才说了算。而 `{"code":110000,"message":"音源获取失败"}` 这类错误响应里本来就没有可取的 URL，仍会在取值失败后报同一条 warn 并计未命中。白名单与字节嗅探的优先级不受影响。
...
- **遍历顺序按会话内健康度动态调整**（ADR 2026-09-25 决策 1–6 / #398）：排序**只改顺序、绝不缩减候选集**——所有可用源仍会被尝试；连续失败 N=2 沉底、成功一次即回归；无样本时完全回退清单顺序（冷启动零行为变化）。**第一首进 tier3 的歌**以「初始化窗口」形态解析（交错起手 H=600ms、在飞 ≤2、沿用上面的按 kind 常态墙、命中即交付、不阻塞播放），整会话至多一个窗口，且窗口内的在飞计入 K=3（全局上游在飞上限恒为 3）。
- **统计与健康度仅会话内**，不持久化、不做熔断/自动禁用/删源；健康度**只影响遍历顺序**（ADR 2026-09-25 决策 2/3 取代 ADR-0014 决策 4 的「不做软降权」一句）。健康度重置时机 = 应用重启 + **订阅变更**；**护栏拒绝**与 **source gate 跳过**不记分（只计数 + trace），**截尾样本**（被单源墙切掉）降权 w=0.5，**放弃观测**（预算用尽 / 用户切走）不进健康度。
