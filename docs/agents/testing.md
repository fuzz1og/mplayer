...
- **Mobile**: `packages/mobile/vitest.config.ts`（node env），setup 只 mock AsyncStorage（`__tests__/setup.ts`）；store 测试用纯 getState/setState。`npx vitest run --config packages/mobile/vitest.config.ts`
