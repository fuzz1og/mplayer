...
**tier3 源健康度**:
会话内按每源观测样本（EWMA α=0.3、reward 含耗时）算出的分数，只用于**调整解析腿的遍历顺序**：连续失败 2 次沉底、成功一次即回归、无样本时回退清单顺序；不熔断、不禁用、不删源、不落盘，订阅变更或应用重启即重置。
_Avoid_: 源评分、源权重、熔断分数

**初始化窗口**:
整会话至多一次的并行解析形态（单飞）：第一首进 tier3 的歌本来那次解析以交错起手（H=600ms）、在飞 ≤2 遍历源，命中即交付、不阻塞播放；不引入任何窗口级墙值（沿用按 kind 常态墙），窗口内的在飞计入跨歌在飞上限 K=3。
_Avoid_: 预热窗口、探测窗口、测速窗口

**截尾样本**:
被单源墙切掉、没测出结果的观测（`SOURCE_TIMED_OUT`）。它不代表源坏了，故按降权 w=0.5 计入健康度；与之相对的「放弃观测」（调用方预算用尽 / 用户切走）不进健康度。
_Avoid_: 超时样本、失败样本
